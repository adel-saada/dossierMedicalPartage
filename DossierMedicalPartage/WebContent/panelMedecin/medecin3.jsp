...
<html lang="fr">
    <head>
        ...
        <title>Medecin - Prescriptions</title>
    ...
    <body>
        ...
        <nav class="menu-gestion">
            <ul >
                ...
                <li><span id="menu-1"><a href="<c:url value="/panelMedecin/medecin3.jsp"/>"><b>Prescriptions</b></a></span></li>
        ...
        <div id="content">
            ...
            <form action="<c:url value="/PanelMedecin"/>" method="POST" id="medecin-form">					
	           	<div class="redefinition-gauche" >
					<input name ="bouton_charger" disabled value ="VOIR CONSULTATIONS DE ${sessionScope.patientSelection.nom}" class="bouton-visualiser-consultation" id="btn-chargement" type ="submit" onclick="actionPost('Consultation',3);" />
						...
						<c:if test="${not empty sessionScope.mapConsulDiagPatientMedecin }">
					        <table class="liste modif-table-consultation">
								...
								<c:forEach items="${ sessionScope.mapConsulDiagPatientMedecin }" var="mapConsultation" varStatus="status">
								    <tr id="${status.count}consultation"  onclick="remplirPrescription('${mapConsultation.value.idDiagnostic}','${mapConsultation.value.prescriptionDiagnostic}','${mapConsultation.key.dateConsultation}','${status.count}consultation', '${ sessionScope.mapConsulDiagPatientMedecin.size() }' );" > 
									    <td> 
	            ...
	            <div class="redefinition-droite">		       			       					
					<div id="prescription-diagnostic">
						<div>
		       			<fieldset>Prescription :</fieldset>
		       			<textarea id="prescription" name="prescription"cols="70" rows="4"></textarea>	
						</div>
						<div class="bloc-date">
		       				<div class="bouton-modifier-diagnostic">
		       				<input name = "bouton_modifier" hidden=true value ="MODIFIER PRESCRIPTION" class="bouton-modifier-consultation" id="btn-modif-prescription" type ="submit" onclick="actionPost('modifierPrescription');" />	       						       				
		       				</div>
						</div>
		...
		<script type="text/javascript">
			...
			*/
			function remplirInput(id) {
				...
				document.getElementById("btn-modif-prescription").hidden = true;
			...
			*/			
			function remplirPrescription(idDiagnostic,prescription, date, idLigne, totalLigne){
						 
				var elementPrescriptionDiagnostic = document.getElementById("prescription");
				var elementBoutonPrescription = document.getElementById("btn-modif-prescription");
				...
				elementBoutonPrescription.hidden = false;
				if(idDiagnostic == 0){
					elementPrescriptionDiagnostic.placeholder = "aucune information";
					elementBoutonPrescription.value = "AJOUTER PRESCRIPTION";

				} else {
					elementPrescriptionDiagnostic.textContent = prescription;
					elementBoutonPrescription.value = "MODIFIER PRESCRIPTION";
